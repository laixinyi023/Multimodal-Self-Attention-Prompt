...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.text_layer = text_layer
        self.img_layer_depth = design_details["SAP_depth"]
        self.text_layer_depth = design_details["SAP_depth"]


        self.text_range = design_details["text_range"]
        self.img_range = design_details["image_range"]
        self.design_details = design_details


        # self.attn = nn.MultiheadAttention(d_model, n_head)
        # self.attn = MultiHeadAttention(d_model, d_model // n_head, d_model // n_head, n_head)

        if self.text_layer:
            if self.i in self.text_range:
                self.attn = MultiHeadAttention(d_model, d_model // n_head, d_model // n_head, n_head,is_text_layer=text_layer,layer=self.i)
            else:
                self.attn = nn.MultiheadAttention(d_model, n_head)
        else:
            if self.i in self.img_range:
                self.attn = MultiHeadAttention(d_model, d_model // n_head, d_model // n_head, n_head,is_text_layer=text_layer,layer=self.i)
            else:
                self.attn = nn.MultiheadAttention(d_model, n_head)


        # if self.i < self.layer_depth:
        #     self.attn = MultiHeadAttention(d_model, d_model // n_head, d_model // n_head, n_head)
        # else:
        #     self.attn = nn.MultiheadAttention(d_model, n_head)
        self.ln_1 = LayerNorm(d_model)
        ...
        # self.x_adapter = nn.Parameter(torch.HalfTensor(1,d_model))
        # nn.init.normal_(self.x_adapter, std=0.02)
    def attention(self, x: torch.Tensor,k = None,v = None):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        ...
        #     v = v.permute(1,0,2)
        #     m = k.shape[0]
        #     k = np.sqrt(self.d_model) * k.expand(m, x.shape[1], self.d_model)
        #     v = np.sqrt(m) * v.expand(m, x.shape[1], self.d_model)
        #
        #     key = torch.cat([x, k], dim=0)
        #     value = torch.cat([x, v], dim=0)
        #
        #
        #     m_mask = None
        #     if self.attn_mask != None:
        #         m_mask = torch.empty(x.shape[0], m)
        #         m_mask.fill_(float(0))
        #         # mask.triu_(1)  # zero out the lower diagonal
        #         m_mask = m_mask.to(self.attn_mask.device)
        #         m_mask = torch.cat([self.attn_mask,m_mask],dim=1)
        #
        #
        #
        #
        #     return self.attn(x, key,value, need_weights=False, attn_mask=m_mask)[0]
        if self.text_layer:
            if self.i in self.text_range:
                return self.attn(x, x, x, k, v, need_weights=False, attn_mask=self.attn_mask)
            else:
                return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
        else:
            if self.i in self.img_range:
    ...
    def forward(self, inputs):
        ...
        # as the shallow version  [x, compound_deeper_prompts, 0, ks, vs, ks_distill, vs_distill, [], [], isnew]
        x = inputs[0]
        ...
        if ks is None:
            ...
            x = x + self.mlp(self.ln_2(x))
            counter = counter + 1
            return [x, compound_prompts_deeper, counter, ks, vs]
        ...
        if self.text_layer:
            ...
            if self.i in self.text_range:
                ...
                # x_distill = x_distill + self.attention(self.ln_1(x_distill),k_distill,v_distill)
                # x_distill = x_distill + self.mlp(self.ln_2(x_distill))
                x = x + self.attention(self.ln_1(x), k, v)
                x = x + self.mlp(self.ln_2(x))
                # x = x + self.x_adapter
                counter = counter + 1
                # memory_distill.append(x_distill)
...
class VisionTransformer_MaPLe(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))
        # self.x_adapter = nn.Parameter(torch.HalfTensor(1,512))

        # nn.init.normal_(self.x_adapter,std=0.02)
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts,ks = None,vs = None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        outputs = self.transformer([x, compound_deeper_prompts, 0, ks, vs])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
        ...
        if self.proj is not None:
            x = x @ self.proj
            # x = x + self.x_adapter
        # for i,image_feature in enumerate(outputs[5]):
        #     image_feature = image_feature.permute(1, 0, 2)
        #     image_feature = self.ln_post(image_feature[:, 0, :])
        #     if self.proj is not None:
        #         image_feature = image_feature @ self.proj
        #     outputs[5][i] = image_feature
        return x
...
class CLIP(nn.Module):
    ...
    def MASP_encode_text(self,text):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        combined = [x, None, 0, None, None]
        x = self.transformer(combined)[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
